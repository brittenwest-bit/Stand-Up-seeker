  ...
  for(const s of sources){
   try{
    ...
    for(const item of items){
      ...
      const show=rows&&rows[0]; if(show){upserted++;await db("show_sources?on_conflict=show_id,source_url",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify({show_id:show.id,source_type:s.source_type,source_name:s.name,source_url:s.url,is_official:true,last_checked_at:new Date().toISOString(),last_result:"confirmed"})}); await db("rpc/refresh_show_verification",{method:"POST",body:JSON.stringify({p_show_id:show.id})});}
    }
